...
Project 1 Tracking of April Tag and projecting image
"""
import numpy as np
import imutils
import cv2

# Read the video file and image to be superimposed.
...
def corners(image): # Use Harris corner detection to define and display corners

    # Corner Detection
    gray = np.float32(image)
    corners = cv2.cornerHarris(gray, 2, 3, 0.04)
    # Dialate frames for creating corners
    corners = cv2.dilate(corners,None)
    # Threshold the corners. Smaller value returns more corners, larger value returns less corners
    frame[corners > .1 * corners.max()] = [0, 0, 255]
...
out = cv2.VideoWriter('tracking.mp4', fourcc, 20.0, (1920, 1080))
...
Multiply H by the testudo image and overlay with the AR tag video.
"""
